...
SELECT player-name, 
       FLOOR(DATEDIFF('2018-12-02', dob) / 365) AS age
FROM player where bowling-skill="Legbreak googly" AND age>28 ORDER BY age DESC,player-name;
select match-id,toss-winner from match where toss-decision="bat" ORDER BY match-id;
SELECT over-id, runs-scored
FROM batsman_scored
